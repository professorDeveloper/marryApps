...
export function DeductionGroupEditView({ isNew = false }: DeductionGroupEditViewProps) {
    ...
    const handleSubmit = useCallback(
        async (data: GroupFormData) => {
            try {
                if (!data.name.trim()) {
                    throw new Error(t('deductions.groupNameRequired', 'Please enter group name'));
                }
                ...
                if (isNew) {
                    ...
                    if (result) {
                        setGroup(result);
                        toast.success(t('deductions.groupCreated', 'Group created successfully'));
                        navigate(paths.warehouse.deductionGroups.root);
                    }
                } else if (id) {
                    ...
                    if (result) {
                        setGroup(result);
                        toast.success(t('deductions.groupUpdated', 'Group updated successfully'));
                        navigate(paths.warehouse.deductionGroups.root);
                    }
                }
            } catch (error) {
    ...
    const config = useMemo(
        () => ({
            title: isNew
                ? t('deductions.createGroup', 'Create New Group')
                : t('deductions.editGroup', 'Edit Group'),
            entityName: 'Deduction Group',
            breadcrumbs: [
                { name: t('app'), href: paths.menu.root },
                { name: t('deductions.groups', 'Groups'), href: paths.warehouse.deductionGroups.root },
                { name: isNew ? t('deductions.new', 'New') : String(group?.name || 'Group'), href: '' },
            ],
            ...
            sections: [
                {
                    id: 'basic-info',
                    title: t('deductions.groupDetails', 'Group Details'),
                    columns: 1,
                    fields: [
                        {
                            key: 'name',
