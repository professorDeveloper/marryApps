...
export type ShipmentsLineItemsApi = {
    getBatchData: () => ShipmentBatchItemInput[];
    restoreFromPersisted: (items: ShipmentBatchItemInput[]) => void;
...
interface ShipmentsLineItemsProps {
    apiRef: React.RefObject<ShipmentsLineItemsApi | null>;
...
export const ShipmentsLineItems = React.memo(function ShipmentsLineItems({
    apiRef,
    ingredients,
    ingredientsLoading,
    ...
    apiRef.current = {
        getBatchData: () => {
            const seen = new Set<string>();
            const batch: ShipmentBatchItemInput[] = [];
            for (const id of transferredIds) {
                if (seen.has(id)) continue;
                seen.add(id);
                batch.push({
