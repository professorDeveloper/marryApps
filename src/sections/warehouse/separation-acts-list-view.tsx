...
export function SeparationActsListView() {
  ...
  const loadSeparationActs = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getSeparationActs(filters);
  ...
  const columns = useMemo<GridColDef[]>(
    () => [
      ...
      {
        field: 'source_ingredient_id',
        headerName: t('separationActs.sourceIngredient', 'Source ingredient'),
        flex: 1,
        minWidth: 170,
        renderCell: (params) => ingredientsMap[params.row.source_ingredient_id] || params.row.source_ingredient_id,
      },
      {
        field: 'storage_id',
      ...
      },
      {
        ...
        getActions: (params) => [
          <CustomGridActionsCellItem
            ...
            onClick={() => router.push(paths.warehouse.separationActs.edit(String(params.row.id)))}
          />,
          <CustomGridActionsCellItem
            key="delete"
    ...
    [storagesMap, groupsMap, ingredientsMap, t, router]
  );
  ...
  return (
    <>
      <GenericTableView
        ...
        breadcrumbs={{
          heading: t('overview.warehouse.separationActs', 'Separation acts'),
          links: [
