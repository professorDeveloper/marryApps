...
export function DeductionsDetailsModal({
  ...
  return (
    <GenericViewModal
      ...
      title={
        data
          ? `${t('deductions.details')} #${data.number}`
          : t('deductions.details')
      }
      data={data}
      loading={loading}
      maxWidth="lg"
          ...
          <Box sx={{ display: 'grid', gap: 2 }}>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>{t('deductions.balance')}</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  <TableRow>
              ...
              <Box sx={{ height: 500, width: '100%' }}>
                <DataGrid
                  ...
                  columns={[
                    ...
                    { field: 'ingredientId', headerName: t('warehouse.ingredient'), flex: 1, renderCell: (params: any) => ingredientsMap[params.value] || params.value },
                    { field: 'quantity', headerName: t('deductions.quantity'), width: 120 },
                    { field: 'pricePerUnit', headerName: t('calculation.pricePerUnit'), width: 150, renderCell: (params: any) => formatAmount(params.value) },
                    { field: 'amount', headerName: t('calculation.remaining'), width: 150, renderCell: (params: any) => formatAmount(params.value) },
                    { field: 'stockBefore', headerName: t('shipments.stockBefore'), width: 120 },
                    { field: 'stockAfter', headerName: t('shipments.stockAfter'), width: 120 },
                  ]}
                  autoHeight
            ...
            {/* <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 3 }}>
              <Typography variant="body2">
                <strong>{t('calculation.remaining')}:</strong> {formatAmount(totalAmount)}
              </Typography>
              <Typography variant="body2">
                <strong>{t('deductions.balance')}:</strong> {formatAmount(deduction.balance)}
              </Typography>
            </Box> */}
          </Box>
        );
