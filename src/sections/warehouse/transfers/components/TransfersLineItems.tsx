...
export type TransfersLineItemsApi = {
    getBatchData: () => TransferBatchItemInput[];
    restoreFromPersisted: (items: TransferBatchItemInput[]) => void;
...
interface TransfersLineItemsProps {
    apiRef: React.RefObject<TransfersLineItemsApi | null>;
...
export const TransfersLineItems = React.memo(function TransfersLineItems({
    apiRef,
    ingredients,
    ingredientsLoading,
    ...
    apiRef.current = {
        getBatchData: () => {
            const seen = new Set<string>();
            const batch: TransferBatchItemInput[] = [];
            for (const id of transferredIds) {
                if (seen.has(id)) continue;
                seen.add(id);
                batch.push({
