...
func (h *Handler) CreateShift(c echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to create shift"})
	}
...
func (h *Handler) GetShiftByID(c echo.Context) error {
	shiftID := c.Param("id")
	if shiftID == "" {
		return c.JSON(http.StatusBadRequest, model.ErrorResponse{Message: "shift id is required"})
	}

	// Validate UUID format
	if _, err := uuid.Parse(shiftID); err != nil {
		return c.JSON(http.StatusBadRequest, model.ErrorResponse{Message: "invalid shift id format"})
	}

	shift, err := h.service.Shift().GetShiftByID(c.Request().Context(), shiftID)
	if err != nil {
		log.Printf("GetShiftByID failed for id %s: %v", shiftID, err)
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to fetch shift"})
	}

	if shift == nil {
		return c.JSON(http.StatusNotFound, model.ErrorResponse{Message: "shift not found"})
	}
...
func (h *Handler) GetAllShifts(c echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to fetch shifts"})
	}
...
func (h *Handler) UpdateShift(c echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to update shift"})
	}
...
func (h *Handler) DeleteShift(c echo.Context) error {
	...
	if err := h.service.Shift().DeleteShift(c.Request().Context(), shiftID); err != nil {
		...
		return c.JSON(http.StatusInternalServerError, model.ErrorResponse{Message: "failed to delete shift"})
	}

	return c.JSON(http.StatusOK, model.SuccessResponse{Message: "Shift deleted successfully"})
}
