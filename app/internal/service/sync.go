...
func (s *SyncS) updateOrderFromPayload(ctx context.Context, orderSvc *OrderS, id uuid.UUID, payload map[string]interface{}) error {
	...
	if req.Status != nil && *req.Status == string(model.OrderStatusPaid) {
		...
		if cashierID != "" {
			if _, err := orderSvc.MarkOrderPaid(ctx, id.String(), cashierID, nil, stringPtrOrNil(paymentType), stringPtrOrNil(discountPercent), stringPtrOrNil(discountAmount), stringPtrOrNil(discountComment), nil, nil, nil, nil); err != nil {
				return err
			}
		} else {
