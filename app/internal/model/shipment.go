...
type ShipmentItemResponse struct {
	...
	TotalAmount  string     `json:"total_amount"`
	StockBefore  string     `json:"stock_before"` // active: live current stock; confirmed: actual before deduction
	StockAfter   string     `json:"stock_after"`  // active: live projected (current-qty); confirmed: actual after deduction
	CreatedAt    *time.Time `json:"created_at,omitempty"`
	UpdatedAt    *time.Time `json:"updated_at,omitempty"`
}

// CreateShipmentRequest creates a shipment header (draft)
...
// UpdateShipmentRequest updates a draft shipment header
...
// UpsertShipmentItemRequest adds or updates an ingredient item in a draft shipment
