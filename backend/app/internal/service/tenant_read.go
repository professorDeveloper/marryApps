...
// withTenantWrite is a shared package-level helper for write tenant-scoped operations.
// It creates a transaction with tenant context (search_path, brand_id, branch_id)
// and executes the provided callback function with tenant queries.
// The transaction is committed if the callback succeeds, otherwise rolled back.
func withTenantWrite(ctx context.Context, repo *repository.Repository, fn func(context.Context, *pg.Queries) error) error {
	brandID, _ := ctx.Value("brand_id").(string)
	brandID = strings.TrimSpace(brandID)
	if brandID == "" {
		return fmt.Errorf("brand_id is missing in context")
	...
	q := pg.New(tx)
	txCtx := repository.WithTenantQueries(ctx, q)
	txCtx = repository.WithTenantTx(txCtx, tx)
